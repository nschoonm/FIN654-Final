...
This group assignment will cover Sessions 7 and 8 (two weeks). We built a simple portfolio in Assignment 3 with equally weighted allocataions to the commodities we traded. We will represent a restaurant. We will compute optimal holdings of risky and risk-free assets for the Markowitz mean-variance model. We will then build a simple financial web application. With this tool we can also explore impact of the extremes of distributions of financial returns on portfolio results. 

### Problem

A restaurant with a several different recipes and menu options wants to optimize their portfolio in the ingrediants markets with entry into the corn business and use of the tramp trade.  Tramp ships are the company's "swing" option without any fixed charter or other constraint. They allow the company flexibility in managing several aspects of freight uncertainty.   They have allocated \$250 million to purchase ingrediants. The company wants us to:

1.	Retrieve and begin to analyze data about potential commodities to diversify into
2.	Compare potential commodities with existing commodities in conventional ingrediant spot markets
...
1.	Product: Ingrediant commodities and freight charters
2.	Ingrediant, Company, and Geography:
...
4.  All ingrediants traded on the Chicago Board of Trade commodity exchanges
...
#colnames(corr.returns) <- c("Corn & Soybean", "Corn & Wheat", "Soybean & Wheat")
...
- *Corn*: daily Corn price (\$/per metric ton)
- *Soybean*: daily Soybean prices (\$/per metric ton)
- *Wheat *: daily Wheat prices (\$/per metric ton)


### Historical data 2012-2016

- Corn has experienced a number of spikes in price and magnitude percentage change.  
- Soybean is less volatile in terms of price and magnitude percentage change
- Wheat experienced some shocks of volatility in 2015 and 2016
...
- The scatterplot illustrates some of the outliers of each of the three metals.  Wheat appears to be more concentrated around the mean but also experienced its share of volatility
...
- There is a strong correlation (.89) between Corn and Soybean.  This was initially expected since both of these metals are used in similar applications or in the creation of cuproCorn alloy which is used for desalinisation due its high resistant to corrosion.  
- CuproCorn is also used in a variety of other applications ranging from minting, armaments, marine engineering, electrical applications, and for the repair of fan blades found in geothermal power plants.  
...
- There is a strong correlation (0.89) between Metal-A (Corn) and Metal-B (Soybean) due to shared applications.
...
- Wheat is a metal which is used in a plethora of industries and markets.  It's relatively stable price is testament to this.  Moving Wheat from Brasil to West Coast USA for aircraft supply chains is a safe, long term freight line that ship owner's can use to mitigate risk.
